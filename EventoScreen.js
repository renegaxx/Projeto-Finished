...
import QRCode from 'react-native-qrcode-svg'; // Importa a biblioteca para QR code
...
const eventsData = [
    ...
    { id: '11', image: require('./assets/fotosEventos/evento11.jpg') },
];
...
const fetchUserFullName = async (userId) => {
    ...
    if (userSnap.exists()) {
        ...
        return userData.fullName; // Certifique-se de que o nome completo do usuário está salvo com a chave 'fullName'
    } else {
...
const EventoScreen = ({ route, }) => {
    ...
    const navigation = useNavigation(); // Inicializar a navegação
    ...
    const [participantes, setParticipantes] = useState([]); // Estado para armazenar os participantes
    ...
    const handleQRCodeClick = (qrCodeValue) => {
        navigation.navigate('QRcodeScreen', {
            qrCodeValue,
            participantes: participantes, // Passa a lista de participantes
        });
    };
    ...
    // UseEffect para buscar os participantes quando o componente for montado
    useEffect(() => {
        const fetchParticipantes = async () => {
            const eventRef = doc(db, 'eventos', eventId); // Busca o evento específico
            const eventDoc = await getDoc(eventRef);

            if (eventDoc.exists()) {
                const data = eventDoc.data();
                setParticipantes(data.participantes || []); // Define os participantes no estado
            } else {
        ...
        fetchParticipantes();
    }, []); // Dependências vazias, ou seja, executa uma vez ao carregar o componente
    ...
    const handlePagar = async () => {
        ...
        if (user) {
            ...
            try {
                ...
                const response = await axios.post('http://192.168.1.6:3000/send-email', {
                    to: email,
                ...
                await updateDoc(eventRef, {
                    participantes: arrayUnion({
                        ...
                        qrCodeValue: participantCode // Salvar o valor gerado para o QR Code
                    })
                });

                // Atualizar o estado de participantes para forçar a renderização do novo QR Code
                setParticipantes(prevState => [
                    ...prevState,
                    { fullName, email, codigo: participantCode, qrCodeValue: participantCode }
                ]);
    ...
    const generateQRCode = (value) => {
        return new Promise((resolve, reject) => {
            const qrCode = <QRCode value={value} size={200} />;
            qrCode.toDataURL((dataUrl) => {
                if (dataUrl) {
                    resolve(dataUrl); // Retorna a URL do QR code gerado
                } else {
                    reject('Erro ao gerar QR Code');
                }
            });
        });
    };
                ...
                <ScrollView contentContainerStyle={styles.scrollContainer}>
                    ...
                    {informacoesVisiveis && (
                        <View style={styles.ticketGarantido}>
                            <Text style={styles.ticketTitle}>{event.titulo}
                            </Text>
                            ...
                            <View style={styles.QRcode}>
                                {user && participantes
                                    .filter(participante => participante.email === user.email) // Filtra o participante que corresponde ao usuário atual
                                    .map((participante, index) => (
                                        <TouchableOpacity key={index} onPress={() => handleQRCodeClick(participante.qrCodeValue)}>
                                            <QRCode
                                                value={participante.qrCodeValue} // Usa o valor gerado para o QR Code
                                                size={80} // Tamanho do QR Code
                                            />
                                        </TouchableOpacity>
                                    ))}
                            </View>
