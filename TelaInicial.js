...
import { View, Text, Image, StyleSheet, FlatList, TouchableOpacity, Animated, ImageBackground, ScrollView, StatusBar, Alert } from 'react-native';
...
const TelaInicial = () => {
  ...
  const [activeTab, setActiveTab] = useState('Perfil', 'Para Você');
  ...
  const handlePress = (tabName) => {
    Animated.sequence([
      ...
      if (tabName === 'MudarPlano') {
        setIsPlan(true); // Alterna para exibir os botões de plano
      } else {
        setIsPlan(false); // Se não for "MudarPlano", exibe as informações de perfil
      }
    });
  };
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          text: 'Sair',
          onPress: () => {
            signOut(auth) // Realiza o logout do Firebase
              .then(() => {
                console.log('Usuário deslogado');
                // Após o logout, você pode redirecionar para a tela de login, por exemplo:
  ...
  const PLANO_BASICO = 'Básico';
  const PLANO_AVANCADO = 'Avançado';
  const PLANO_PREMIUM = 'Premium';
        ...
        >
          ...
          <View style={styles.slideGay}>
            {!isPlan ? (
              ...
              // Exibe os 3 botões para planos
              <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'center', alignItems: 'center' }}>
                <TouchableOpacity style={styles.buttonSlide}>
                  <Image
                    source={require('./assets/selos/básico.png')}
                    style={styles.planoImage}
                  />
                  <Text style={{ textAlign: 'center', fontFamily: 'Raleway-SemiBold', color: '#fff' }}>Básico</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.buttonSlide}>
                  <Image
                    source={require('./assets/selos/avançado.png')}
                    style={styles.planoImage}
                  />
                  <Text style={{ textAlign: 'center', fontFamily: 'Raleway-SemiBold', color: '#fff' }}>Avançado</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.buttonSlide}>
                  <Image
                    source={require('./assets/selos/premium.png')}
                    style={styles.planoImage}
                  />
                  <Text style={{ textAlign: 'center', fontFamily: 'Raleway-SemiBold', color: '#fff' }}>Premium</Text>
                </TouchableOpacity>
              </View>
            )}
          </View>
          ...
          <View style={styles.slideBotoes}>
            ...
            <TouchableOpacity
              ...
              onPress={handleLogout}  // Aqui você chama a função de logout
            >
              <Image
